 ...
 * target database AND empty the SlabPlan uploads prefix in object
 ...
 *   - Storage deletion is scoped to the SlabPlan uploads prefix only
 *     (`slabplan/uploads/`). Anything outside that prefix is NOT touched.
...
async function planAndWipeBucket(targetLabel) {
  ...
  console.log(
    `\n[${targetLabel}/storage] Bucket=${storage.bucketName} Prefix=${slabPlanUploadsPrefix}`,
  );
  ...
  // Verify.
  const after = await storage.listAllObjects(slabPlanUploadsPrefix);
  if (after.length > 0) {
    throw new Error(
      `Bucket still has ${after.length} objects under ${slabPlanUploadsPrefix} after delete.`,
    );
  }
  console.log(
