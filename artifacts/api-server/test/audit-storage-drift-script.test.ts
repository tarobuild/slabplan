...
test("uploadsObjectPrefix: includes trailing slash and slabplan/uploads/", async () => {
  const mod = await loadScript();
  const prefix = mod.uploadsObjectPrefix();
  assert.equal(prefix, "slabplan/uploads/");
});
...
test("fileUrlToObjectName: round-trips with objectNameToFileUrl", async () => {
  ...
  assert.equal(
    objectName,
    "slabplan/uploads/job-1/photo/1700000000000-uuid-foo.jpg",
  );
...
test("objectNameToFileUrl: returns null for objects outside the SlabPlan uploads prefix", async () => {
  const mod = await loadScript();
  ...
  assert.equal(
    mod.objectNameToFileUrl({
      objectName: "slabplan/restore-drill/roundtrip-1.bin",
    }),
    null,
  );
  assert.equal(
