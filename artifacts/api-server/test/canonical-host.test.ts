...
test("normalizeHostHeader strips ports from host headers", () => {
  assert.equal(normalizeHostHeader("slabplan.vercel.app:443"), "slabplan.vercel.app");
...
test("production canonical host guard allows public canonical hosts", () => {
  ...
  assert.equal(
    isAllowedProductionHost(
      "www.slabplan-api-production.up.railway.app",
      "203.0.113.10",
    ),
    true,
  );
...
test("production canonical host guard allows configured deployment hosts", () => {
  const env = {
    APP_ORIGIN: "https://cadstone-works-tool.replit.app",
    RAILWAY_PUBLIC_DOMAIN: "slabplan-api-staging.up.railway.app",
  ...
  assert.deepEqual(collectConfiguredProductionHosts(env).sort(), [
    "cadstone-works-tool.replit.app",
