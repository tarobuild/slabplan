  ...
  const stub: AnthropicMessages = {
    create: async (args: unknown) => {
      ...
      if (userText.includes("Schedule of Values:")) {
        ...
        // the real ids.
        const match = userText.match(/Schedule of Values:\s*(\[[\s\S]*?\])\s*\n/);
        if (match) {
          try {
...
function pdfMultipart(name: string) {
...
test("GET /jobs/:id/financials lazily creates an empty tracker", async () => {
  ...
  const body = (await res.json()) as {
    tracker: { id: string };
    areas: unknown[];
    totals: { scheduledValueCents: number; billedCents: number };
  };
...
test("POST /financials/estimate parses AI response into SOV areas", async () => {
    ...
    areas: [
      {
        ...
        floor: "Floor 1",
        lineItems: [
          { description: "Quartz Slab A", qty: 1, rateCents: 200_000, scheduledValueCents: 200_000 },
          { description: "Quartz Slab B", qty: 2, rateCents: 150_000, scheduledValueCents: 300_000 },
        ],
      },
    ],
  };
  ...
  const body = JSON.parse(rawBody) as {
    areas: Array<{ id: string; name: string; lineItems: Array<{ id: string; description: string }> }>;
...
test("POST /financials/invoices applies AI matches to line items", async () => {
  ...
  assert.equal(res.status, 201, rawBody);
  const body = JSON.parse(rawBody) as {
    totals: { billedCents: number; scheduledValueCents: number; percentBilled: number };
    invoices: Array<{ id: string; invoiceNumber: string | null; appliedAt: string | null }>;
  };
...
test("DELETE /financials/invoices/:id reverses payments", async () => {
  ...
  assert.equal(after.totals.billedCents, 0, "deleting the invoice must reverse billed_cents");
});
...
test("GET /clients/:id rolls up tracker totals (not legacy job money fields)", async () => {
  ...
  await fetch(`${baseUrl}/api/jobs/${jobId}/financials/line-items/${lineItemA}`, {
    method: "PATCH",
  ...
  const apply = await fetch(`${baseUrl}/api/jobs/${jobId}/financials/invoices`, {
    method: "POST",
    headers: authedHeaders(m.headers),
    body: m.body,
  });
...
test("PATCH line-item scheduledValueCents preserves billed (caps only)", async () => {
  ...
  const before = await fetch(`${baseUrl}/api/jobs/${jobId}/financials`, {
    ...
    areas: Array<{ lineItems: Array<{ id: string; billedCents: number; scheduledValueCents: number; percentComplete: number }> }>;
  }>);
  ...
  const after1 = JSON.parse(r1Text) as {
    lineItem: { billedCents: number; scheduledValueCents: number; percentComplete: string };
  };
  assert.equal(
