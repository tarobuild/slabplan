...
describe("hasRoleAccess", () => {
  test("admin can access admin/project_manager-only routes", () => {
    assert.equal(hasRoleAccess("admin", ROLE_GATES.sales), true)
    assert.equal(hasRoleAccess("admin", ROLE_GATES.clients), true)
  })

  test("project_manager can access admin/project_manager-only routes", () => {
    assert.equal(hasRoleAccess("project_manager", ROLE_GATES.sales), true)
    assert.equal(hasRoleAccess("project_manager", ROLE_GATES.clients), true)
  })

  test("crew_member is blocked from admin/project_manager-only routes", () => {
    // The role gate must redirect crew members away from these routes,
