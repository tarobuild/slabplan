...
test("missing logs drill-down warns when the dashboard sample is incomplete", () => {
  assert.match(
    source,
    /data\.atRisk\.jobsMissingLogs\s*>\s*data\.atRisk\.samples\.missingLogJobs\.length/,
  )
  assert.match(source, /Showing \{data\.atRisk\.samples\.missingLogJobs\.length\}[\s\S]*?sampled[\s\S]*?jobs out of \{data\.atRisk\.jobsMissingLogs\}/)
  assert.match(source, /Return to Home or\s+reports for the full cohort/)
})
