...
export default function JobFilesVideosPage() {
  ...
  const canView = job?.access?.videos ?? true
  if (isFieldUser) {
    ...
    <div className="space-y-4">
      ...
      {canView ? (
        <FileBrowser
          mediaType="video"
          defaultView="grid"
          canUpload={job?.access?.uploadVideos}
          canCreateFolders={job?.access?.createFolders}
        />
      ) : (
        <p className="text-sm text-slate-500">You do not have access to job videos.</p>
      )}
    </div>
  )
}
