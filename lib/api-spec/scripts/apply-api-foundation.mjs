...
function calendarDateQuerySchema() {
  return {
    type: "string",
    pattern: "^\\d{4}-\\d{2}-\\d{2}$",
  };
...
const QUERY_PARAMS_BY_PATH = {
  ...
  "/jobs": [
    ...
    { name: "status", schema: { type: "string", enum: ["open", "closed", "archived"] }, description: "Optional status filter." },
    { name: "clientId", schema: { type: "string", format: "uuid" }, description: "Optional client filter; restricts results to jobs for the given client." },
    { ref: "#/components/parameters/CursorParam" },
    { ref: "#/components/parameters/CursorLimitParam" },
  ],
  "/clients": [
    { name: "page", schema: { type: "integer", minimum: 1 }, description: "Page number (1-based) for offset pagination." },
    { name: "pageSize", schema: { type: "integer", minimum: 1, maximum: 100 }, description: "Page size for offset pagination." },
    { name: "search", schema: { type: "string" }, description: "Optional free-text filter across company name, email, and city." },
    { name: "status", schema: { type: "string", enum: ["active", "archived", "all"] }, description: "Status filter: active (default, not soft-deleted), archived (soft-deleted), or all." },
  ],
  "/leads": [
    { name: "page", schema: { type: "integer", minimum: 1 }, description: "Page number (1-based) for offset pagination. Ignored when `cursor` is supplied." },
    { name: "pageSize", schema: { type: "integer", minimum: 1, maximum: 100 }, description: "Page size for offset pagination. Ignored when `cursor` is supplied." },
  ...
  "/jobs/{jobId}/daily-logs": [
    ...
    { name: "from", schema: calendarDateQuerySchema(), description: "Inclusive lower bound on log date (YYYY-MM-DD)." },
    { name: "to", schema: calendarDateQuerySchema(), description: "Inclusive upper bound on log date (YYYY-MM-DD)." },
  ...
  "/dashboard/schedule": [
    { name: "start", schema: calendarDateQuerySchema(), description: "Inclusive lower bound on the schedule range (YYYY-MM-DD). Defaults to today." },
    { name: "end", schema: calendarDateQuerySchema(), description: "Inclusive upper bound on the schedule range (YYYY-MM-DD). Defaults to today + 60 days. Must be on or after `start`." },
  ],
  "/search": [
